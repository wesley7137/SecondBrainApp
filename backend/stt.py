...
stt_model = WhisperModel("distil-medium.en", device=device, compute_type=compute_type)
    ...
    async def transcribe_worker(audio_file_path):
        try:
            segments, info = await asyncio.get_event_loop().run_in_executor(
                thread_pool, lambda: stt_model.transcribe(audio_file_path)
            )
...
async def stt(audio_data, language=None):
    return await stt_async(audio_data, language)



async def stt_async(audio_data, language=None):
    """
    Async wrapper for speech-to-text conversion
    """
    logger.info(f"Speech-to-text request received. Provider: {STT_PROVIDER}")

    try:
        if STT_PROVIDER == 'local':
            return await stt_local_async(audio_data, language)
        else:
            logger.error(f"Invalid speech-to-text provider: {STT_PROVIDER}")
            raise ValueError(f"Invalid speech-to-text provider: {STT_PROVIDER}")
    except Exception as e:
        if "Invalid file format" in str(e):
            logger.error("Error in speech-to-text: Invalid audio format. Please ensure the audio is in a supported format.")
        else:
